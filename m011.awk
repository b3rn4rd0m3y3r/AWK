...
END {
	...
	for(mes=1;mes<=12;mes++){
		for(dia=1;dia<=28;dia++){
			...
			if( wkday != "Sun" ){
				# Sorteia o nÃºmero de vendas que teremos neste dia
				# Sábado as vendas aumentam, pois é o dia que as
				# pessoas tem maior disponibilidade para comprar
				if( wkday == "Sat" ){
					Max = 11
					} else {
					Max = 6
					}
				VendasDia = random(2,Max)
				for(vnd=1;vnd<=VendasDia;vnd++){
					NoVenda++
					# Sorteia a quantidade deste produto que serÃ¡ vendida
					Quant = sprintf("%d",random(1,4))
					# Sorteia o produto que serÃ¡ vendido
